...
public class ExportService {
     ...
     */
    public void executeExport(ExportRequest request, OutputStream outputStream, String sessionId) throws Exception {
        ...
        securityValidator.validateTableExistence(request.getDatabase(), request.getSchema(), request.getTable(),
                sessionId);
        String safeTableName = securityValidator.getSafeTableName(request.getDatabase(), request.getSchema(),
                request.getTable(), sessionId);
        ...
        try (Connection conn = jdbcTemplate.getDataSource().getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                log.info("Query ejecutada en {}ms. Iniciando escritura de datos...",
    ...
    private SqlQueryBuilder buildSqlQuery(ExportRequest request, String safeTableName) {
        ...
        switch (request.getScope()) {
            case PAGE:
                ...
                int offset = p * ps;
                sql.append(" ORDER BY (SELECT NULL) OFFSET ").append(offset).append(" ROWS FETCH NEXT ").append(ps)
                        .append(" ROWS ONLY");
                break;
            case PAGES:
                ...
                sql.append(" ORDER BY (SELECT NULL) OFFSET ").append(off).append(" ROWS FETCH NEXT ").append(total)
                        .append(" ROWS ONLY");
                break;
            case ROWS:
            ...
            case RANGE:
                ...
                sql.append(" ORDER BY (SELECT NULL) OFFSET ").append(rStart).append(" ROWS FETCH NEXT ").append(rCount)
                        .append(" ROWS ONLY");
                break;
            case ALL:
            default:
